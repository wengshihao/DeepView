...
def RUN(T, isgaus, runtype):
    ...
    model = ssd300_vgg16(num_classes=21, score_thresh=int(T) / 100)  #
    checkpoint = torch.load('./models/SSD_VOC_orgmodel_5.pth', map_location="cpu")
    ...
    with torch.no_grad():
        ...
        if isgaus:
            with open(runtype + 'gaus_SSD_VOC' + T + '.json', 'w') as json_file:
                json_file.write(json_str)
        else:
            with open(runtype + 'SSD_VOC' + T + '.json', 'w') as json_file:
                json_file.write(json_str)

if __name__ == "__main__":
    gaus_list = {False, True}
#     for gaus in gaus_list:
    RUN(T='000', isgaus=False, runtype='RQ3')
