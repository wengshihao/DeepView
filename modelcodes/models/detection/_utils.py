...
class BoxCoder:
    ...
    def decode(self, rel_codes: Tensor, boxes: List[Tensor], flag=None):
        torch._assert(
        ...
        if flag == 'flag':
            pred_boxes, dis_score = self.decode_single(rel_codes, concat_boxes, flag)
        else:
            pred_boxes = self.decode_single(rel_codes, concat_boxes, flag)
        if box_sum > 0:
            pred_boxes = pred_boxes.reshape(box_sum, -1, 4)
            if flag == 'flag':
                dis_score = dis_score.reshape(box_sum, -1, 8)
        if flag == 'flag':
            return pred_boxes, dis_score
        else:
            return pred_boxes

    def decode_single(self, rel_codes: Tensor, boxes: Tensor, flag=None):
        """
        ...
        # modify
        if flag == 'flag':
            # dis = torch.sqrt((pred_ctr_x - ctr_x.unsqueeze(1)) ** 2 + (pred_ctr_y - ctr_y.unsqueeze(1)) ** 2)

            org_c_to_c_h = (torch.tensor(0.5, dtype=ctr_y.dtype, device=heights.device) * heights).unsqueeze(1)
            org_c_to_c_w = (torch.tensor(0.5, dtype=ctr_x.dtype, device=widths.device) * widths).unsqueeze(1)

            xmin1, ymin1, xmax1, ymax1 = ctr_x.unsqueeze(1) - org_c_to_c_w, ctr_y.unsqueeze(
                1) - org_c_to_c_h, ctr_x.unsqueeze(1) + org_c_to_c_w, ctr_y.unsqueeze(1) + org_c_to_c_h
            xmin1, ymin1, xmax1, ymax1 = xmin1.repeat(1, pred_ctr_x.shape[1]), ymin1.repeat(1, pred_ctr_x.shape[
                1]), xmax1.repeat(1, pred_ctr_x.shape[1]), ymax1.repeat(1, pred_ctr_x.shape[1])
            c_to_c_h = torch.tensor(0.5, dtype=pred_ctr_y.dtype, device=pred_h.device) * pred_h
            c_to_c_w = torch.tensor(0.5, dtype=pred_ctr_x.dtype, device=pred_w.device) * pred_w

            xmin2, ymin2, xmax2, ymax2 = pred_ctr_x - c_to_c_w, pred_ctr_y - c_to_c_h, pred_ctr_x + c_to_c_w, pred_ctr_y + c_to_c_h

            dis_score = torch.stack((xmin1, ymin1, xmax1, ymax1, xmin2, ymin2, xmax2, ymax2), dim=2).flatten(1)

            # xx1 = torch.tensor([xmin1[i][0].item() if xmin1[i][0] > xmin2[i][0] else xmin2[i][0].item() for i in
            #                     range(xmin1.shape[0])], device=heights.device)
            #
            # yy1 = torch.tensor([ymin1[i][0].item() if ymin1[i][0] > ymin2[i][0] else ymin2[i][0].item() for i in
            #                     range(ymin1.shape[0])], device=heights.device)
            #
            # xx2 = torch.tensor([xmax1[i][0].item() if xmax1[i][0] < xmax2[i][0] else xmax2[i][0].item() for i in
            #                     range(xmax1.shape[0])], device=heights.device)
            # yy2 = torch.tensor([ymax1[i][0].item() if ymax1[i][0] < ymax2[i][0] else ymax2[i][0].item() for i in
            #                     range(ymax1.shape[0])], device=heights.device)
            # area1 = (xmax1 - xmin1 + 1) * (ymax1 - ymin1 + 1)
            # area2 = (xmax2 - xmin2 + 1) * (ymax2 - ymin2 + 1)
            # inter_area = torch.tensor(
            #     [xx2[i] - xx1[i] if xx2[i] - xx1[i] > 0 else 0 for i in range(xx1.shape[0])],
            #     device=heights.device) * torch.tensor(
            #     [yy2[i] - yy1[i] if yy2[i] - yy1[i] > 0 else 0 for i in range(yy1.shape[0])], device=heights.device)
            # dis_score = inter_area.unsqueeze(1) / (area1 + area2 - inter_area.unsqueeze(1) + 1e-6)
            # dis_score = box_ops.box_iou(org_box, new_box)
        #
