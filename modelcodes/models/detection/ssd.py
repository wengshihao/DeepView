...
class SSD(nn.Module):
    ...
    def postprocess_detections(
        ...
        for boxes, scores, anchors, image_shape in zip(bbox_regression, pred_scores, image_anchors, image_shapes):
            boxes, dis_scores = self.box_coder.decode_single(boxes, anchors, flag='flag')
            boxes = box_ops.clip_boxes_to_image(boxes, image_shape)
            dis_scores = torch.squeeze(dis_scores, dim=1)
            image_boxes = []
            image_scores = []
            image_labels = []
            image_full_score = []
            ...
            detections.append(
                {
                    ...
                    "dis_scores": image_dis_score[keep]
                }
            )
        return detections
