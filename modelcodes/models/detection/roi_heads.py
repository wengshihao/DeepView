...
##new_function
'''
def fastrcnn_loss_test(class_logits, box_regression, labels, regression_targets):
    ...
    classification_loss = F.cross_entropy(F.softmax(class_logits, -1), labels, reduction="none")
...
class RoIHeads(nn.Module):
    ...
    def postprocess_detections(
            ...
            # loss_classifier,  # type: Tensor
            # loss_box_reg,  # type: Tensor
    ):

        # type: (...) -> Tuple[List[Tensor], List[Tensor], List[Tensor],List[Tensor],List[Tensor]] #modify

        device = class_logits.device
        num_classes = class_logits.shape[-1]

        boxes_per_image = [boxes_in_image.shape[0] for boxes_in_image in proposals]
        pred_boxes, dis_score = self.box_coder.decode(box_regression, proposals, 'flag')  # dis_score

        pred_scores = F.softmax(class_logits, -1)

        pred_boxes_list = pred_boxes.split(boxes_per_image, 0)
        pred_scores_list = pred_scores.split(boxes_per_image, 0)
        dis_score_list = dis_score.split(boxes_per_image, 0)
        # loss_classifier_list = loss_classifier.split(boxes_per_image, 0)
        # loss_box_reg_list = loss_box_reg.split(boxes_per_image, 0)
        ...
        all_labels = []
        full_scores = []
        all_p_max_100_use4entropy = []
        for boxes, scores, dis_scores, image_shape in zip(pred_boxes_list, pred_scores_list, dis_score_list,
                                                          image_shapes):
            ...
            ###modify
            indrow, indcol = torch.where(scores[:, 1:] > self.score_thresh)
            indcol = indcol + 1
            boxes, scores, one_scores, dis_scores, labels = boxes[indrow, indcol], scores[indrow], scores[
                indrow, indcol], dis_scores[indrow, indcol], labels[indrow, indcol]

            keep = box_ops.remove_small_boxes(boxes, min_size=1e-2)

            boxes, scores, one_scores, dis_scores, labels = boxes[keep], scores[keep], one_scores[keep], dis_scores[
                keep], \
                                                            labels[keep]
            # print(boxes.shape, scores.shape, labels.shape)

            keep = box_ops.batched_nms(boxes, one_scores, labels, self.nms_thresh)
            ...
            boxes, scores, one_scores, dis_scores, labels = boxes[keep], scores[keep], one_scores[keep], dis_scores[
                keep], \
                                                            labels[keep]
            # print(boxes.shape, scores.shape, one_scores.shape, labels.shape, cls_loss.shape, box_loss.shape)

            all_boxes.append(boxes)
            all_scores.append(one_scores)
            all_dis_score.append(dis_scores)
            all_labels.append(labels)
            full_scores.append(scores)

        return all_boxes, all_scores, all_labels, full_scores, all_dis_score
    ...
    def forward(
        ...
        if self.training:
            ...
            # changed
            '''
            loss_classifier, loss_box_reg = fastrcnn_loss_test(class_logits, box_regression, labels, regression_targets)
            losses = {"loss_classifier": loss_classifier, "loss_box_reg": loss_box_reg}
            '''
            ##########

            boxes, scores, labels, full_scores, dis_scores = self.postprocess_detections(  # dis_score
                class_logits,
                box_regression,
                proposals,
                image_shapes, )
                ...
                result.append(
                    {
                        ...
                        "labels": labels[i],
                        "scores": scores[i],
                        "full_scores": full_scores[i],  # modify
                        # "cls_loss": cls_loss[i],  # modify
                        # "box_loss": box_loss[i],  # modify
                        # "p_max_100_use4entropy": p_max_100_use4entropy[i],  # modify
                        "dis_scores": dis_scores[i]
                    }
                )
